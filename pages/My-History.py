...
@st.cache_resource
def get_all_imports():
    from core import userActions, chatActions, rantSessions
    from streamlit_chat import message
    import utils
    from config import OPENAI_API_KEY
    return userActions, chatActions, rantSessions, message, utils, OPENAI_API_KEY

userActions, chatActions, rantSessions, message, utils, OPENAI_API_KEY = get_all_imports()
...
def display_session_chat(chats, count):
    ...
    if count > 0:
        with chat_container:
            for chat in chats:
                if chat["message"] != "":
                    message(utils.remove_stage_from_message(chat["message"]), is_user=chat["type"]=="user")
    else:
...
if 'uid' not in st.session_state:
    ...
    selection = st.sidebar.selectbox("Go to (Select between sessions and reports)", ["Assessment Reports", "Sessions"])
